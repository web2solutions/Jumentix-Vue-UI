...
export default {
  ...
  methods: {
    ...
    async feedItems (entity) {
      ...
      if (entity === 'Case') {
        let { data, total, error } = await getFromApi(entity, this, null, 'status');
        if (data.length > 0) {
          data.forEach(caseItem => {
            if (caseItem.status) {
              let status = `${caseItem.status.label.toString()[0].toUpperCase()}${caseItem.status.label.toString().slice(1)}`;

              this.dataset.caseTypes.forEach((element, i) => {
                if (element.name === status) return this.dataset.caseTypes[i].value++;
              });
            }
          });
        }
        // this.$refs.caseChart.pie.init();
      }
