...
import { setFormFields, getGridWidth, getFormType, getGridColAlign, getOnLocalCollection, getLocalCollection } from '../../../helpers/helpers';
import session from './../../../helpers/session';
import xFormGrid from './xFormGrid.vue';
import UploadButton from './../../../components/UploadButton.vue';
export default {
  name: 'xGrid',
  props: ['currentFormMode', 'forms', 'dialog', 'entity', 'selected', 'totalDocuments', 'headers', 'original_documents', 'documents', 'loading', 'itemKey', 'bus', 'displayWhat'],
  components: {
    xFormGrid,
  ...
  mounted () {
    ...
    // console.log(this.headers, this.documents);
  ...
  methods: {
    ...
    getTasks (_id, fieldId) {
      window.setTimeout(() => {
        // console.log('DDDDDDDDDDDDDDD', _id);
        // console.log('DDDDDDDDDDDDDDD field', fieldId);
        this._getTasks(_id, fieldId);
      }, 700);
    },
    _getTasks (_id, fieldId) {
      // console.log('DDDDDDDDDDDDDDD', _id);
      // console.log('DDDDDDDDDDDDDDD field', fieldId);
      if (!_id)
      {
      ...
      (async (elId) => {
        try {
          let { error, data } = await getOnLocalCollection('Case', _id);
          let tasksTotal = 0;
          data.phase.forEach(async phases => {
            let phaseData = await getOnLocalCollection('Phase', phases.phase);
            tasksTotal += phaseData.data.task.length;
            // console.log('Phase', phaseData, elId, total, tasksTotal);
            document.getElementById(elId).innerHTML = tasksTotal;
          });
        } catch (error) {
          // console.log(error);
          console.warn('error on filling field', fieldId);
