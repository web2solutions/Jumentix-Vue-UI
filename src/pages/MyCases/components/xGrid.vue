      ...
      >
        ...
        <td 
          ...
          <template v-else>
            ...
            <v-tooltip top>
              <v-btn small icon class="mx-0 ml-3 mt-2" slot="activator" :to="'/my-cases/case/' + props.item._id">
                <v-icon small>preview</v-icon>
              </v-btn>
              <span>View {{ props.item.case_number + ' ' + props.item.display_name}}</span>
            </v-tooltip>

            <v-tooltip top>
              <v-btn small icon class="mx-0 mt-2" slot="activator" @click="editItem(props.index)">
                <v-icon small>edit</v-icon>
              </v-btn>
              <span>Edit {{ props.item.case_number + ' ' + props.item.display_name}}</span>
            </v-tooltip>

            <v-tooltip top>
              <v-btn small icon class="mx-0 mt-2" slot="activator" @click="deleteItem(props.item)">
                <v-icon small>delete</v-icon>
              </v-btn>
              <span>Delete {{ props.item.case_number + ' ' + props.item.display_name}}</span>
            </v-tooltip>

           <!--  <v-tooltip top>
              <v-btn small icon class="mx-0 mt-2" slot="activator" :to="'contact-details/' + props.item._id">
        ...
        <td width="100%" align="center">
          <div :id="'task_' + props.item._id + '_' + props.index">
            <v-progress-linear :indeterminate="true"></v-progress-linear>
          </div>
          {{ getTasks(props.item._id, 'task_' + props.item._id + '_' + props.index) }}
        </td>
