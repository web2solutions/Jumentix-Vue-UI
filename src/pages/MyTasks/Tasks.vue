<template>
  <v-container grid-list-xl fluid py-0>
    ...
    <vue-perfect-scrollbar :class="[ session.user().role !== 'parent' && session.user().role !== 'child' ? 'perfect--scrollbar--task' : 'perfect--scrollbar--task--parent']">
      <template v-for="(item, index) in dataItem[0]">
                              ...
                              <v-layout>
                                <v-flex xs7>
                                  <v-layout align-start justify-start column fill-height>
                                    ...
                                    <v-flex xs12 pt-0 class="grey--text">
                                      <h2 class="body-1 font-weight-light">{{ props.item.phase }}</h2>
                                      <h2 class="body-1 font-weight-light">Total Task: {{ props.item.total }}</h2>
                                    </v-flex>
                                  </v-layout>
                                </v-flex>
                                ...
                                <v-flex xs5 pb-0 mb-0>
                                  <v-layout align-end justify-end row fill-height>
                                    <v-btn small color="primary">See Tasks</v-btn>
                                  </v-layout>
                                </v-flex>
                              </v-layout>
                            </v-card-text>
              ...
              <v-card slot-scope="{ hover }" :class="`elevation-${hover ? 8 : 1}`">
                ...
                <v-card-text>
                  ...
                  >
                    ...
                    <span class="caption d-block">{{ item.phase.length }} / {{ item.taskTotal }}</span>
                  </v-progress-circular>
                </v-card-text>
                <v-card-actions>
