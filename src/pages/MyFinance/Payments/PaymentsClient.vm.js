...
export default {
  ...
  methods: {
    feedGrid (url) {
      ...
      let where = { 'drawee_human': this.customerID };
      ...
      (async () => {
        ...
        this.invoiceItems = data.map(doc => {
          console.log('doc', doc.status, doc.id);
          ...
          for (let prop in this.swagger.definitions[this.entity].properties) {
            if (this.swagger.definitions[this.entity].properties.hasOwnProperty(prop)) {
              ...
              switch (prop) {
                case 'transaction':
                  if (doc[prop] != null) {
                    newDoc['payment_number'] = ''; // reset
                    newDoc['payment_mode'] = '';
                        
                    (async () => {
                      let { error, data, status } = await getOne('Transaction', doc[prop]);
                      if (error) {
                      ...
                      {
                        // console.log(data);
                        newDoc['payment_number'] = data.info.transaction_number;
                        newDoc['payment_mode'] = data.transaction_type;
