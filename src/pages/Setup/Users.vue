<template>
  <v-container id="UsersTop">
            ...
            <v-toolbar flat color="white" class="pr-0">
              ...
              <v-dialog v-model="usersDialog" scrollable max-width="650px">
                <template v-slot:activator="{ on }">
                  <v-btn color="primary" small dark class="mb-2" v-on="on">New users</v-btn>
                </template>
                <v-card>
                  <v-card-title>
                  ...
                  <v-card-text>
                        ...
                        <v-layout wrap>
                          ...
                          <v-flex xs12 md6>
                            ...
                            >
                              ...
                              <v-date-picker v-model="human.birthDate" @input="humanPicker = false"></v-date-picker>
                            </v-menu>
                          </v-flex>
                    ...
                    <v-layout row wrap py-3>
                      <v-flex xs6>
                        ...
                        >
                          <template v-slot:items="props">
                            <tr :active="props.selected" @click="rowClick(props.item)">
                  ...
                  <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn color="blue darken-1" flat @click="close('users')">Cancel</v-btn>
                    <v-btn color="blue darken-1" flat @click="save('users')">Save</v-btn>
                  </v-card-actions>
                </v-card>
              </v-dialog>
            </v-toolbar>
            <v-data-table :headers="usersHeaders" :items="usersItems" class="elevation-1" hide-actions>
              <template v-slot:items="props">
                <td>{{ props.item.first_name }}</td>
                <td>{{ props.item.user ? props.item.user.username : '' }}</td>
                <td>{{ props.item.user ? props.item.user.roles ? props.item.user.roles[0] : props.item.user.roles : '' }}</td>
                <td>
                  <v-icon small class="mr-2" @click="editItem(props.item, 'users')">edit</v-icon>
                  <v-icon small @click="deleteItem(props.item, 'users')">delete</v-icon>
                </td>
              </template>
              <template v-slot:no-data>
                <div class="text-xs-center">
                  <v-btn small flat color="primary" @click="usersDialog = true">Add a new users</v-btn>
                </div>
              </template>
            </v-data-table>
          </v-flex>
    ...
    <v-btn color="primary" @click="bus.$emit('nextStep', 6)">Continue</v-btn>
    <v-btn flat>Cancel</v-btn>
  </v-container>
</template>
<script>
import Users from './Users.vm.js';
