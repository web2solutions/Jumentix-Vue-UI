                  ...
                  <v-data-table 
                    ...
                    >
                      <template v-slot:headers="props">
                        <tr>
                          <th width="20" v-if="session.user().role !== 'parent'">
                            <v-checkbox
                          ...
                          <th
                            align="center" v-if="session.user().role !== 'parent'"
                          >
                            Action
                          </th>
                      </tr>
                    ...
                    <template v-slot:items="props">
                      <tr :active="props.selected" @click="props.selected = !props.selected">
                        <td v-if="session.user().role !== 'parent'">
                          <v-checkbox
                        ...
                        <td v-if="session.user().role !== 'parent' && session.user().role !== 'child'" class="text-xs-center px-0"><router-link :to="'/summary/'+ props.item.customer_id">{{props.item.customer_name}}</router-link></td>
                        <td class="text-xs-center px-0" :class="props.item.statusColor">{{ props.item.status }}</td>
                        <td class="text-xs-center px-0">{{ moment(new Date(props.item.due_date).toISOString().substr(0, 10)).format('L') }}</td>
                        <td class="text-xs-center">{{ props.item.amount != null ? props.item.amount : '$0.00' }}</td>
                        <td v-if="session.user().role !== 'parent'" class="text-xs-center px-0">
                          <v-tooltip top>
                            <v-icon small slot="activator" @click="goTo('invoice/'+props.item._id)">remove_red_eye</v-icon>
                            <span>View Invoice</span>
                          </v-tooltip>
                          <v-tooltip top>
                            <v-icon small slot="activator" @click="goTo('invoice-edit/' + props.item._id)" :disabled="props.item.status === 'Paid'">edit</v-icon>
                          ...
                          <v-tooltip top>
                            <v-icon small slot="activator" @click="goTo('invoice-payment/' + props.item._id)" :disabled="props.item.status === 'Paid'">fas fa-file-invoice-dollar</v-icon>
...
<script>
import Invoices from './Invoices.vm.js';
