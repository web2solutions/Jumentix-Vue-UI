...
export default {
  ...
  watch: {
    themeColor: {
      handler (val) {
        ...
        (async () => {
          ...
          let result = await update('User', payload, this.session.user._id);
          // console.log(result);
        })();
      },
      immediate: true
    ...
    session: {
      handler () {
        // if (!this.session.user) return;
        // console.log('watched session XXXXXXXXXXXX', this.session);
        // this.setAvatar();
        // this.setName();
        if (this.session.user)
        {
          if (this.session.user.portal_color) this.$vuetify.theme.primary = this.colors[this.session.user.portal_color].base;
          this.$vuetify.dark = this.session.user.portal_isDark ? true : false;
        } 
      },
      deep: true
    }
  ...
  mounted () {
    // console.log('THEME', this.session);
    if (this.session.user)
    {
      if (this.session.user.portal_color)
      {
        this.$vuetify.theme.primary = this.colors[this.session.user.portal_color].base;
        this.themeColor = this.session.user.portal_color;
      } 
      this.$vuetify.dark = this.session.user.portal_isDark ? true : false;

      
      this.sideBarOption = this.session.user.portal_isDark ? 'dark' : 'light';
    } 
  },
};
</script>
