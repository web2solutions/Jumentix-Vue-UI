          ...
          >
            ...
            <template v-else-if="getFormType(getProperty(schemaName, headerName)) === 'select'">
              <template v-if="getFieldCollectionSettings(getProperty(schemaName, headerName))">
                  <div :id="'sel_xform_' + headerName + '_' + props.index">

                      <v-progress-linear :indeterminate="true"></v-progress-linear>
            
                    </div>
                    {{ fillField(getFieldCollectionSettings(getProperty(schemaName, headerName)), props.item[headerName], 'sel_xform_' + headerName+ '_' + props.index) }}
                </template>
                <template v-else>
                  {{ props.item[headerName] }}
              </template>
            ...
            <template v-else-if="getFormType(getProperty(schemaName, headerName)) === 'currency'">
              {{ parseFloat(props.item[headerName]).toFixed(2) }}
            </template>
            <template v-else>
              {{ props.item[headerName] }}
...
<script>
import xSimpleFormGrid from './xSimpleFormGrid.vm.js';
export default xSimpleFormGrid;
</script>
